import { Box, Heading, List, ListItem, Text, Stack, Button, Flex, Modal, ModalOverlay, ModalContent, ModalHeader, ModalFooter, ModalBody, ModalCloseButton, Input, useDisclosure, useToast } from '@chakra-ui/react';
...
function MyPlaylistsPage() {
  ...
  useEffect(() => {
    ...
    let config = {
      ...
      url: 'http://localhost:8080/playlists/my-playlists',
      headers: { 
        'Authorization': `${token}`,
      }
    };
  ...
  const handleAllPlaylists = () => {
    navigate('/playlists');
  };

  return (
    <Box p={8}>
      <Heading mb={4}>My Playlists</Heading>
      {token ? (
        <Flex mb={6} justifyContent="space-between">
          <Button colorScheme="blue" onClick={onOpen} leftIcon={<AddIcon />}>
            Add New Playlist
          </Button>
          <Button colorScheme="teal" onClick={handleAllPlaylists}>
            All Playlists
          </Button>
        </Flex>
      ) : null}
